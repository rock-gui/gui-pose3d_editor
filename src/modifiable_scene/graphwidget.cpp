...
bool GraphicsWindowQt::init()
{
    ...
    Qt::WindowFlags flag = Qt::Window|Qt::CustomizeWindowHint;//|Qt::WindowStaysOnTopHint;
    if ( _traits->windowDecoration )
        // flag |= Qt::WindowTitleHint|Qt::WindowMinMaxButtonsHint|Qt::WindowCloseButtonHint;
        flag |= Qt::WindowTitleHint|Qt::WindowMinMaxButtonsHint|Qt::WindowSystemMenuHint;

    _widget = new GraphWidget( format, 0, 0, flag );
...
void GraphicsWindowQt::grabFocusIfPointerInWindow()
{
    if ( _widget )
    {
        ...
        if ( _widget->geometry().contains(pos) )
            _widget->setFocus( Qt::ActiveWindowFocusReason );
    }
}
...
void GraphicsWindowQt::setCursor( MouseCursor cursor )
{
    ...
    case CrosshairCursor: _currentCursor = Qt::CrossCursor; break;
        // case HandCursor: _currentCursor = Qt::PointingHandCursor; break;
...
bool GraphicsWindowQt::realizeImplementation()
{
    ...
    // A makeCurrent()/doneCurrent() semms to be required for
